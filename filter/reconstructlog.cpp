...
#include "macros.h"
#include "memalloc.h" // ideal_merge_my_malloc
...
static log_rel_t * log_rel_init(uint64_t nrels)
{
    log_rel_t * rels;
    uint64_t i;
...
static void log_rel_free(log_rel_t * rels, uint64_t nrels)
{
    uint64_t i;
    for (i = 0; i < nrels; i++)
...
static light_rels_t light_rels_init(uint64_t nrels)
{
    light_rels_t rels;
...
static void light_rels_free(light_rels_t rels)
{
    free(rels);
}
...
struct logtab {
    ...
    struct mpz_rw_accessor : public mpz_ro_accessor {
      ...
      public:
        ...
        mpz_rw_accessor & operator=(mpz_srcptr v)
        {
            if (is_known()) {
                gmp_fprintf(stderr,
                            "ERROR, inconsistent log for h = %" PRIu64
                            " ; we previously had %Zd in the database, now we "
                            "want to store %Zd\n",
                            h, (mpz_srcptr) * this, v);
            ...
            if (mpz_cmp_ui(v, 0) < 0) {
                fprintf(stderr,
                        "Warning, log is negative for h = %" PRIu64 "\n", h);
                cxx_mpz vv;
                ...
                if (mpz_cmp_ui(v, 0) == 0) {
                    fprintf(stderr,
                            "Warning, log is zero for h = %" PRIu64 "\n", h);
                } else {
  ...
  public:
    ...
    logtab(cado_poly_srcptr cpoly, std::vector<sm_side_info> const & sm_info,
        ...
        , sm_info(sm_info)
    {
        ...
        data = (mp_limb_t *)malloc((nprimes + nbsm) * mpz_size(ell) *
                                   sizeof(mp_limb_t));
...
struct read_data {
    ...
    read_data(logtab & log, uint64_t nrels, cado_poly_ptr cpoly,
    ...
    ~read_data() { log_rel_free(rels, nrels); }
};
 ...
 * value) */
static inline weight_t nb_unknown_log(read_data & data, uint64_t i)
{
 ...
 */
static inline unsigned int compute_missing_log(logtab::mpz_rw_accessor dest,
...
static uint64_t log_do_one_part_of_iter(read_data & data, bit_vector not_used,
        ...
        if (bit_vector_getbit(not_used, (size_t)i)) {
            ...
            if (nb <= 1) {
                ...
                if (nb == 0 && mpz_cmp_ui(vlog, 0) != 0) {
                    gmp_fprintf(stderr,
                                "Error, no unknown log in rel %" PRIu64
                                " and sum"
                    ...
                    computed += compute_missing_log(data.log[ideal.id], vlog,
                                                    ideal.e, data.log.ell);
                }
            }
        }
    }
...
static graph_dep_t graph_dep_init(uint64_t size)
{
    node_dep_t * tab = NULL;
...
static void graph_dep_clear(graph_dep_t G)
{
    free(G.tab);
}

/* Set G[h].state accordingly to log[h] values */
static void graph_dep_set_log_already_known(graph_dep_t G, logtab const & log)
{
...
static uint64_t graph_dep_needed_rels_from_index(graph_dep_t G, index_t h,
                                                 light_rels_t rels,
                                                 bit_vector needed_rels)
{
    if (G.tab[h].state == NODE_DEP_LOG_UNKNOWN) {
        fprintf(stderr,
                "Error: logarithms of %" PRid " cannot be reconstructed "
        ...
        for (weight_t j = 0; j < nb_needed; j++) {
            ...
            if (!bit_vector_getbit(needed_rels, G.tab[hh].i)) {
                nadded +=
                    graph_dep_needed_rels_from_index(G, hh, rels, needed_rels);
            }
        }
        return nadded;
    }
...
struct dep_read_data {
    light_rels_t rels;
    graph_dep_t G;
    dep_read_data(light_rels_t rels, graph_dep_t G)
        : rels(rels)
        , G(G)
    {
    }
 ...
 * logarithms is 1) */
static inline weight_t dep_nb_unknown_log(dep_read_data & data, uint64_t i,
                                          index_t * h)
{
    weight_t k, nb;
...
static uint64_t dep_do_one_part_of_iter(dep_read_data & data,
                                        bit_vector not_used, uint64_t start,
                                        uint64_t end)
{
    uint64_t i, computed = 0;
...
static uint64_t do_one_iter_mt(void * data, bit_vector not_used, int nt,
                               uint64_t nrels, int version)
{
    ...
    while ((i < nrels) || (active_threads > 0)) {
        ...
        if ((active_threads < nt) && (i < nrels)) {
            ...
            pthread_create(&threads[threads_head], NULL, &thread_start,
                           (void *)(&tis[threads_head]));
            i += SIZE_BLOCK;
            active_threads++;
            threads_head++;
            if (threads_head == nt)
 ...
 * Return the number of computed logarithms */
static inline uint64_t log_do_one_iter_mt(read_data & d, bit_vector bv, int nt,
                                          uint64_t nrels)
{
 ...
 * Multi thread version
 * Return the number of computed logarithms */
static inline uint64_t dep_do_one_iter_mt(dep_read_data & d, bit_vector bv,
                                          int nt, uint64_t nrels)
{
    ...
    for (; i < tab.get_size(); i++) {
        ...
        if (x.side != tab.get_rational_side())
            gmp_fprintf(f, "%" PRid " %" PRpr " %d %" PRpr " %Zd\n", (index_t)i,
                        x.p, x.side, x.r, (mpz_srcptr)log[i]);
        else
            gmp_fprintf(f, "%" PRid " %" PRpr " %d rat %Zd\n", (index_t)i, x.p,
                        x.side, (mpz_srcptr)log[i]);
        if (stats_test_progress(stats))
                                      ...
                                      int nt, read_data & data)
{
    ...
    if (!filename_matches_one_compression_format(relspfilename) &&
        ...
        printf(
            "# Files %s and %s are uncompressed, limiting consumer threads\n",
    ...
    wct_tt0 = wct_seconds();
    do {
        printf("# Iteration %" PRIu64 ": starting... [%" PRIu64
               " known logs]\n",
                                    ...
                                    char const * wanted_filename, int nt)
{
    ...
    do {
        ...
        printf("# Iteration %" PRIu64 ": %" PRIu64
               " new dependencies computed\n",
               iter, computed);
        printf("# Iteration %" PRIu64 " took %.1fs (wall-clock time).\n", iter,
               wct_seconds() - wct_tt);
    ...
    while (fscanf(f, "%" SCNid "\n", &h) == 1) {
        ...
        fflush(stdout);
        nadded =
            graph_dep_needed_rels_from_index(dep_graph, h, rels, needed_rels);
        nrels_necessary += nadded;
        printf("-> %" PRIu64 " needed relations were added (%" PRIu64
               " so far)\n",
