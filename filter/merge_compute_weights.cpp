...
struct bucket : public std::vector<uint16_t> {
    ...
    /* unfortunately we can't be sure to reuse the same vecotr for both
     * cache and updates, since the type isn't a sure match
     */
    std::vector<col_weight_t> cache;
    ...
    {
        if (last_flush == SIZE_MAX) {
            for(unsigned int i = 0 ; i < max && i < cache.size() ; i++)
                dst[i] += cache[i];
        } else {
            for(auto pos : *this)
                dst[pos]++;
    ...
    void flush(col_weight_t * dst, size_t when, int T, std::mutex & mm) {
         ...
         */
        if (when < last_flush + 16 * T) {
            /* convert this bucket to a cache */
            fprintf(stderr,
                    "# thread %d converts writes to %p from async to last\n",
    ...
    inline void push(uint16_t j, col_weight_t * dst, size_t when, int T, std::mutex & mm) {
        if (last_flush == SIZE_MAX) {
            /* this bucket has been converted to a local cache
             * because there are frequent flushes in this range
             */
            cache[j]++;
        } else {
            push_back(j);
