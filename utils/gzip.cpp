...
int is_supported_compression_format(char const * s)
{
...
int filename_matches_one_compression_format(char const * path)
{
    for (auto const & r: cado::details::supported_compression_formats) {
        if (!*r.suffix)
            continue;
        if (has_suffix(path, r.suffix))
            return 1;
    }
    return 0;
}

void get_suffix_from_filename(char const * s, char const ** sfx)
{
    for (auto const & r: cado::details::supported_compression_formats) {
...
int set_antebuffer_path(char const * executable_filename,
                        char const * path_antebuffer)
{
    *antebuffer = 0;
    ...
    if (path_antebuffer) {
        ...
        if (rc < 0) {
            ...
            if (S_ISDIR(sbuf->st_mode)) {
                ...
                snprintf(antebuffer, PATH_MAX - 1,
                         "%s/antebuffer" EXECUTABLE_SUFFIX, path_antebuffer);
#else
                snprintf(antebuffer, PATH_MAX - 1, "%s/antebuffer",
                         path_antebuffer);
#endif
            } else {
                strncpy(antebuffer, path_antebuffer, PATH_MAX - 1);
            }
            if (try_antebuffer_path())
                return 1;
        }
    }
    ...
    if (executable_filename) {
        ...
        char const * slash = strrchr(executable_filename, '/');
        if (slash) {
...
#ifdef EXECUTABLE_SUFFIX
        snprintf(dummy2, sizeof(dummy2),
                 "%s/../utils/antebuffer" EXECUTABLE_SUFFIX, dummy);
#else
...
prepare_grouped_command_lines(std::vector<std::string> const & list_of_files)
{
    ...
    for (auto it = list_of_files.begin(); it != list_of_files.end();) {
        ...
        const struct cado::details::suffix_handler * this_suffix = nullptr;
        for (auto const & r: cado::details::supported_compression_formats) {
            if (it->ends_with(r.suffix)) {
                this_suffix = &r;
                break;
            }
        }
        ...
        if (*antebuffer) {
            if (*this_suffix->pfmt_in) {
                ...
                cmd_prefix =
                    fmt::format("{} {}", antebuffer, antebuffer_buffer_size);
                cmd_postfix = fmt::format(
                    " | {}",
                    fmt::format(fmt::runtime(this_suffix->pfmt_in), "-"));
            } else {
        ...
        for (; it != list_of_files.end(); ++it) {
            const struct cado::details::suffix_handler * other_suffix = nullptr;
            for (auto const & r: cado::details::supported_compression_formats) {
                if (it->ends_with(r.suffix)) {
                    other_suffix = &r;
                    break;
...
FILE * fopen_maybe_compressed2(char const * orig_name, char const * mode,
                               int * p_pipeflag, char const ** suf)
{
    FILE * f;
...
FILE * fopen_maybe_compressed(char const * name, char const * mode)
{
    return fopen_maybe_compressed2(name, mode, nullptr, nullptr);
}

#ifdef HAVE_GETRUSAGE
int fclose_maybe_compressed2(FILE * f, const char * orig_name,
                             struct rusage * rr)
#else
/* if we don't even have getrusage, then no fclose_maybe_compressed2 is
 * exposed. Yet, we use one as a code shortcut
 */
static int fclose_maybe_compressed2(FILE * f, char const * orig_name,
                                    void * rr MAYBE_UNUSED)
#endif
{
    std::string name = orig_name;
    ...
    for (auto const & r: cado::details::supported_compression_formats) {
        ...
        std::string const tempname = fmt::format("{}.tmp.{}", name, getpid());
...
int fclose_maybe_compressed(FILE * f, char const * name)
{
