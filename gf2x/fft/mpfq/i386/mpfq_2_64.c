...
mpz_srcptr mpfq_2_64_field_characteristic_srcptr(mpfq_2_64_src_field K MAYBE_UNUSED)
{
        /* yes, this is ugly */
...
int mpfq_2_64_vec_cmp(mpfq_2_64_dst_field k, mpfq_2_64_src_vec u, mpfq_2_64_src_vec v, unsigned int n)
{
        for (unsigned int i = 0; i < n; ++i) {
            mpfq_2_64_src_elt x = mpfq_2_64_vec_coeff_ptr_const(k, u, i);
            mpfq_2_64_src_elt y = mpfq_2_64_vec_coeff_ptr_const(k, v, i);
        int ret = mpfq_2_64_cmp(k, x, y);
        if (ret != 0)
            return ret;
        }
        return 0;
...
int mpfq_2_64_vec_is_zero(mpfq_2_64_dst_field k, mpfq_2_64_src_vec v, unsigned int n)
{
        for (unsigned int i = 0; i < n; ++i) {
            mpfq_2_64_src_elt x = mpfq_2_64_vec_coeff_ptr_const(k, v, i);
        if (!mpfq_2_64_is_zero(k, x))
            return 0;
        }
        return 1;
}
...
int mpfq_2_64_vec_hamming_weight(mpfq_2_64_dst_field k, mpfq_2_64_src_vec v, unsigned int n)
{
        int w = 0;
        for (unsigned int i = 0; i < n; ++i) {
            mpfq_2_64_src_elt x = mpfq_2_64_vec_coeff_ptr_const(k, v, i);
        w += !mpfq_2_64_is_zero(k, x);
        }
        return w;
}

/* *Mpfq::defaults::vec::generic::code_for_vec_find_first_set, Mpfq::defaults::vec */
int mpfq_2_64_vec_find_first_set(mpfq_2_64_dst_field k, mpfq_2_64_src_vec v, unsigned int n)
{
        for (unsigned int i = 0; i < n; ++i) {
            mpfq_2_64_src_elt x = mpfq_2_64_vec_coeff_ptr_const(k, v, i);
        if (!mpfq_2_64_is_zero(k, x))
