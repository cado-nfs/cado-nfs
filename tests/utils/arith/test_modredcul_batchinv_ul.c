...
static void print_residues_ul(char const * prefix, unsigned long const * a,
                              size_t len, unsigned long const m)
{
    if (!verbose)
        return;
    printf("%s: ", prefix);
...
/* Returns 1 if a1[i]*a2[i] == 1, for 0 <= i < len,
   returns 0 otherwise. */
static int check_product_c_ul(unsigned long * a1, unsigned long * a2,
                              unsigned long const orig_c, size_t const len,
                              modulusredcul_t const m)
{
    int ok = 1;
    mpz_t prod;
    unsigned long const c = orig_c % modredcul_getmod_ul(m);
    ...
    for (size_t i = 0; i < len; i++) {
        ...
        if (r != c) {
            ok = 0;
            fprintf(stderr,
                    "Inverse is incorrect: %lu * %lu = %lu != %lu (mod %lu)\n",
...
static unsigned long choose_odd_modulus_ul(unsigned long const i)
{
    switch (i) {
    case 0:
        return 3;
...
static unsigned long choose_constant(unsigned long const i,
...
static int test_modredc_batchinv_ul(size_t const len, unsigned long const c,
                                    modulusredcul_t const m)
{
    int ok = 1;
    ...
    unsigned long * r = (unsigned long *)malloc(len * sizeof(unsigned long));
    ...
    for (size_t i = 0; i < len; i++) {
        unsigned long g = gcd_ul(a[i], modredcul_getmod_ul(m));
        /* May have to divide out factors with greater multiplicity in a than
           they have in m */
        while (g != 1) {
            had_common_factor = 1;
            /* Divide out gcd so we can try again with a set of residues for
               which the inverses exist */
            a[i] /= g;
    ...
    if (had_common_factor && rc != 0) {
        fprintf(stderr,
                "Input numbers had common factor but modredcul_batchinv()"
                " returned %d",
                rc);
        ok = 0;
        goto finish;
    ...
    print_residues_ul("Input numbers after dividing by gcd", a, len,
                      modredcul_getmod_ul(m));
    rc = modredcul_batchinv_ul(r, a, c, len, m);
    if (rc == 0) {
        fprintf(stderr,
                "After dividing out common factors, modredcul_batchinv()"
                " still returned 0\n");
        ok = 0;
        goto finish;
    }
...
static int check_Q_to_Fp(unsigned long const q, unsigned long const num,
                         unsigned long const den, unsigned long const k,
                         unsigned long const p)
{
    ...
    if (!ok) {
        gmp_fprintf(stderr,
                    "Error, %lu != %Zd for num=%lu, den=%lu, k=%lu, p=%lu\n", q,
                    r, num, den, k, p);
    }
...
static int
check_modredcul_batch_Q_to_Fp(unsigned long * r, unsigned long const num,
                              unsigned long const den, unsigned long const k,
                              unsigned long const * p, size_t const n)
{
...
static int test_modredcul_batch_Q_to_Fp(unsigned long const num,
                                        unsigned long const den,
                                        unsigned long const k, size_t const n)
{
...
int main(int argc, char const * argv[])
{
    ...
    for (unsigned long i = 0; ok && i < iter; i++) {
        ...
        for (size_t len = 0; len < 10; len++) {
            ...
            unsigned long const nr_c = 20;
            for (unsigned long k = 0; k < nr_c; k++)
                ok &=
                    test_modredc_batchinv_ul(len, choose_constant(k, nr_c), m);
        }
        modredcul_clearmod(m);
    }

    for (unsigned long i = 1; ok && i < iter; i++) {
        unsigned long const den = choose_odd_modulus_ul(i);
        unsigned long const nr_c = 20;
        for (unsigned long j = 0; ok && j < nr_c; j++) {
            unsigned long const num = choose_constant(i, nr_c);
