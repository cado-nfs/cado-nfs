...
if [ "${ELL}" ] ; then
    "${BUILD_DIR}/numbertheory/badideals" -poly "${POLY}" -ell "$ELL" -badidealinfo ${WORKDIR}/badidealinfo -badideals ${WORKDIR}/badideals
    COMPAT=(-badideals ${WORKDIR}/badideals)
fi
...
"${BUILD_DIR}/misc/debug_renumber" "${common[@]}" -check
if [ "${ELL}" ] ; then
    common=(-renumber "${RENUMBER}")
    COMPAT=(-badidealinfo ${WORKDIR}/badidealinfo)
fi
"${BUILD_DIR}/filter/dup2" "${common[@]}"       \
                    -nrels $(zcat "$WORK_RELS" | wc -l) "${DL[@]}" "${COMPAT[@]}" "${WORK_RELS}"
