...
struct plingen_tuning_cache {
    ...
    struct coarse_compare {
        bool operator()(size_t const &a, size_t const& b) const {
            size_t ca = round_operand_size(a);
            size_t cb = round_operand_size(b);
            return ca < cb;
        }
        bool operator()(basecase_key const &a, basecase_key const& b) const {
            basecase_key ca = a;
            basecase_key cb = b;
            std::get<3>(ca) = round_operand_size(std::get<3>(ca));
            std::get<3>(cb) = round_operand_size(std::get<3>(cb));
            return ca < cb;
        }
        bool operator()(mul_key const &a, mul_key const& b) const {
            mul_key ca = a;
            mul_key cb = b;
            std::get<1>(ca) = round_operand_size(std::get<1>(ca));
            std::get<2>(ca) = round_operand_size(std::get<2>(ca));
            std::get<1>(cb) = round_operand_size(std::get<1>(cb));
            std::get<2>(cb) = round_operand_size(std::get<2>(cb));
            return ca < cb;
        }
