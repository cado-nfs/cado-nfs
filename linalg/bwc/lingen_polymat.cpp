...
#endif
    /* }}} */
    void reducemat(polymat & c, unsigned int kc, unsigned int ka); 
    void addmulmat(
            unsigned int kc,
...
void polymat::multiply_column_by_x(unsigned int j, unsigned int nsize)/*{{{*/
{
        ...
        polymat const & src, unsigned int jsrc, unsigned int ksrc)
{
    ...
    bwmat_copy_coeffs(ab,
                part(0, jdst, kdst), n,
            src.part(0, jsrc, ksrc), src.n, src.m);
}/*}}}*/
