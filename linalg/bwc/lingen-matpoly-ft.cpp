...
#pragma omp parallel for collapse(2)
    ...
    for(unsigned int i = 0 ; i < m ; i++) {
        for(unsigned int j = 0 ; j < n ; j++) {
            size_t offset = (i*n+j) * fft_alloc_sizes[0];
            void * tij = pointer_arith(data, offset);
...
void matpoly_ft::dft(matpoly const & a)
{
    size_t fft_alloc_sizes[3];
    fft_get_transform_allocs(fft_alloc_sizes, fti);
...
void matpoly_ft::add(matpoly_ft const & t0, matpoly_ft const & t1)
{
    /* Recall that this is for polynomials ! */
    ASSERT_ALWAYS(t0.m == t1.m);
    ASSERT_ALWAYS(t0.n == t1.n);
    ASSERT_ALWAYS(t0.m == m);
    ASSERT_ALWAYS(t0.n == n);
...
#pragma omp parallel for collapse(2)
    ...
    for(unsigned int i = 0 ; i < t0.m ; i++) {
        for(unsigned int j = 0 ; j < t0.n ; j++) {
            ...
            const void * t0ij = pointer_arith_const(t0.data, offset);
            const void * t1ij = pointer_arith_const(t1.data, offset);
            void * uij  = pointer_arith(data, offset);
            fft_add(uij, t0ij, t1ij, fti);
        }
    }
}

void matpoly_ft::addmul(matpoly_ft const & t0, matpoly_ft const & t1)
{
    ASSERT_ALWAYS(t0.n == t1.m);
    ASSERT_ALWAYS(t0.m == m);
    ASSERT_ALWAYS(t1.n == n);
    size_t fft_alloc_sizes[3];
    fft_get_transform_allocs(fft_alloc_sizes, fti);
        ...
        for(unsigned int i = 0 ; i < m ; i++)
            for(unsigned int j = 0 ; j < n ; j++) {
                ...
                for(unsigned int k = 0 ; k < t0.n ; k++) {
                    void * uij  = pointer_arith(data, (i*n+j) * tsize);
                    const void * t0ik = pointer_arith_const(t0.data, (i*t0.n+k) * tsize);
                    const void * t1kj = pointer_arith_const(t1.data, (k*t1.n+j) * tsize);
...
void matpoly_ft::mul(matpoly_ft const & t0, matpoly_ft const & t1)
{
    zero();
    addmul(t0, t1);
}
...
void matpoly_ft::ift_mp(matpoly & a, unsigned int shift)
{
    ASSERT_ALWAYS(m == a.m);
    ASSERT_ALWAYS(n == a.n);
    size_t fft_alloc_sizes[3];
...
void matpoly_mul_caching_adj(matpoly & c, matpoly const & a, matpoly const & b, unsigned int adj, const struct lingen_substep_schedule * S MAYBE_UNUSED)/*{{{*/
{
    ...
    c = matpoly(a.ab, a.m, b.n, csize);
    matpoly_ft ta(c.ab, a.m, a.n, fti);
    matpoly_ft tb(c.ab, b.m, b.n, fti);
    matpoly_ft tc(c.ab, a.m, b.n, fti);
    ta.dft(a);
    tb.dft(b);
    tc.mul(ta, tb);
    c.size = csize;
...
void matpoly_mp_caching_adj(matpoly & c, matpoly const & a, matpoly const & b, unsigned int adj, const struct lingen_substep_schedule * S MAYBE_UNUSED)/*{{{*/
{
    struct fft_transform_info fti[1];
    ...
    c = matpoly(a.ab, a.m, b.n, MAX(a.size, b.size) - MIN(a.size, b.size) + 1);
    matpoly_ft ta(c.ab, a.m, a.n, fti);
    matpoly_ft tb(c.ab, b.m, b.n, fti);
    matpoly_ft tc(c.ab, a.m, b.n, fti);
    ta.dft(a);
