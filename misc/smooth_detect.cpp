...
void
cand_set(cand_t c, const cand_t c0)
{
    mpz_set(c->u0, c0->u0);
    mpz_set(c->v0, c0->v0);
    mpz_set(c->u, c0->u);
...
void
cand_init_copy(cand_t c, const cand_t c0)
{
    cand_init(c);
    cand_set(c, c0);
}
...
bool
cand_is_factored(const cand_t c)
{
...
bool
cand_is_probably_not_smooth(const cand_t c, unsigned int bound)
{
    double eff = c->effort;
    unsigned int bits = 0;
...
void
cand_update_check_prime(cand_t c)
{
...
void
cand_set_original_values(cand_t c,
                         const mpz_t u0,
                         const mpz_t v0,
                         unsigned long id)
{
...
void
cand_set_presieved_values(cand_t c,
                          const mpz_t u0,
                          const mpz_t v0,
                          const mpz_t u,
                          const mpz_t v,
...
// For use in qsort
int
cand_cmp(const void* c1, const void* c2)
{
    cand_t* C1 = (cand_t*)c1;
    cand_t* C2 = (cand_t*)c2;
    if (cost(*C1) > cost(*C2))
...
void
pool_insert(pool_t p, const cand_t c)
{
...
void
pool_purge(pool_t p, unsigned int max_size, unsigned int lmax)
{
    unsigned int i, j;
    j = 0;
    for (i = 0; i < p->n; ++i) {
...
typedef struct
{
    ...
    unsigned long
      nb_test[MAX_CPT]; // size of the sample on which this avearge was done
} stats_s;
typedef stats_s stats_t[1];

void
stats_init(stats_t S)
{
...
void
stats_update(stats_t S, double gain, unsigned int i)
{
    if (i >= MAX_CPT) {
        abort();
    }
...
int
stats_is_below_average(stats_t S, double gain, unsigned int i)
{
    if (S->nb_test[i] <= 100)
...
void
my_ecm_factor(mpz_t f, mpz_t z, double B1)
{
    ecm_params ecm_par;
    ecm_init(ecm_par);
    long sig = random();
...
int
smooth_detect_one_step(cand_t winner, context_t ctx)
{
    cand_t C;
    cand_init(C);
    ...
    while (!cand_is_probably_not_smooth(C, ctx->target) &&
           !cand_is_factored(C) && C->effort < ctx->current_effort) {
        cpt++;
        // u-side
