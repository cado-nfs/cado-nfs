 ...
 */
static void
ropt_quadratic_tune_stage1 ( ropt_poly & poly,
  ...
  )
{
  ...
  for (i = bound->global_w_boundl; i <= bound->global_w_boundr; i++) {
    ...
    old_i = rotate_aux (poly.cpoly[1], poly.cpoly[0], old_i, i, 2);
  ...
  old_i = rotate_aux (poly.cpoly[1], poly.cpoly[0], old_i, 0, 2);
  ropt_poly_setup (poly);
  ...
  for (i = 0; i < numw; i ++) {
    ...
    old_i = rotate_aux (poly.cpoly[1], poly.cpoly[0], old_i, w, 2);
    ropt_poly_setup (poly);
