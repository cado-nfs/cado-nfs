...
polyselect_data_t polyselect_data_opt_lognorm, polyselect_data_exp_E,
    polyselect_data_beta, polyselect_data_eta, polyselect_data_best_exp_E;
...
static inline size_t expected_number_of_pairs()
{
  uint32_t P = Primes[0];
...
static double expected_collisions()
{
#if 0
...
/* check that l <= m0/P^2 where l = p1 * p2 * q with P <= p1, p2 <= 2P
   ...
   l for degree d-2. */
static int check_parameters(mpz_srcptr m0, double q)
{
...
static double get_ad_double(unsigned long idx)
{
...
print_poly_info(char *buf,
		...
		mpz_srcptr n,
		const int raw,
		const char *prefix, bool raw_option, unsigned long idx)
{
...
#pragma omp critical
#endif
      polyselect_data_add(raw_proj_alpha,
...
#pragma	omp critical
#endif
      polyselect_data_add(opt_proj_alpha,
			  get_alpha_projective(f, get_alpha_bound()));
    }
  ...
  if (!raw_option && (target_E != 0.0 || maxtime < DBL_MAX))
      ...
      if (target_E != 0.0)
	{
	  ...
	  np += snprintf(buf + np, size - np,
			 ...
			 polyselect_data_exp_E->size,
			 polyselect_data_exp_E->min,
			 polyselect_data_mean(polyselect_data_exp_E),
			 polyselect_data_exp_E->max,
			 sqrt(polyselect_data_var(polyselect_data_exp_E)));
	  np +=
	      snprintf(buf + np, size - np,
...
*/
static int optimize_raw_poly(mpz_poly_ptr f, mpz_poly_ptr g)
{
  double skew;
  mpz_t t;
...
static unsigned long get_idx(mpz_srcptr ad)
{
  mpz_t t;
  unsigned long idx;
  mpz_init_set(t, ad);
...
find_suitable_lq(polyselect_poly_header_srcptr header,
		 polyselect_qroots_srcptr SQ_R, unsigned long *k)
{
...
collision_on_p(polyselect_poly_header_srcptr header,
	       ...
	       polyselect_shash_ptr H, gmp_randstate_ptr rstate)
{
  ...
  if (verbose > 2)
    {
      ...
      fprintf(stderr,
	      "# polyselect_shash_size (umax = %" PRId64 ", P = %" PRId32
...
collision_on_each_sq(polyselect_poly_header_srcptr header,
		     ...
		     unsigned long q,
		     mpz_srcptr rqqz,
		     unsigned long *inv_qq, polyselect_shash_ptr H)
{
  ...
  if (found)
      ...
      for (nprimes = c = 0; nprimes < lenPrimes; nprimes++)
	{
	  ...
	  for (j = 0; j < nr; j++, c++)
		...
		polyselect_hash_add(H, p, v2, header->m0, header->ad,
...
collision_on_each_sq_r(polyselect_poly_header_srcptr header,
		       ...
		       int count, polyselect_shash_ptr H)
{
  if (count == 0)
    return;
...
/* find collisions between "P" primes, return number of loops */
static inline unsigned long
gmp_collision_on_p(polyselect_poly_header_srcptr header,
		   polyselect_proots_ptr R, gmp_randstate_ptr rstate)
{
  ...
  for (nprimes = 0; nprimes < lenPrimes; nprimes++)
      ...
      if (polyselect_poly_header_skip(header, p))
	{
      ...
      nrp =
	  roots_mod_uint64(rp, mpz_fdiv_ui(header->Ntilde, p), header->d, p,
			   rstate);
...
gmp_collision_on_each_sq(polyselect_poly_header_srcptr header,
			 polyselect_proots_srcptr R,
			 uint64_t q, mpz_srcptr rqqz, uint64_t * inv_qq)
{
  unsigned int nr, j;
  ...
  for (nprimes = 0; nprimes < lenPrimes; nprimes++)
      ...
      for (j = 0; j < nr; j++, c++)
	{
	  ...
	  for (v = u; v < umax; v += ppl)
	    polyselect_hash_add_gmp(H, p, v, header->m0, header->ad,
				    header->d, header->N, q, rqqz);
...
gmp_collision_on_sq(polyselect_poly_header_srcptr header,
		    polyselect_proots_srcptr R,
		    unsigned long c, gmp_randstate_ptr rstate)
{
  ...
  while (i <= (tot - BATCH_SIZE))
	...
	gmp_fprintf(stderr, "q: %lu, qq: %Zd, rqq: %Zd\n",
      ...
      // collision batch
      gmp_collision_on_batch_sq(header, R, q, (const mpz_t *) qqz,
				(const mpz_t *) rqqz, BATCH_SIZE, c);
      i += BATCH_SIZE;
    }
  ...
  gmp_collision_on_batch_sq(header, R, q, (const mpz_t *) qqz,
			    (const mpz_t *) rqqz, tot % BATCH_SIZE, c);
...
collision_on_batch_sq_r(polyselect_poly_header_srcptr header,
			...
			unsigned long k, polyselect_shash_ptr H)
{
  int count;
  ...
  while (re)
      ...
      for (count = 0; count < BATCH_SIZE; count++)
	{
      ...
      collision_on_each_sq_r(header, R, q, (const mpz_t *) rqqz, inv_qq,
			     number_pr, num_rq, H);
    }
...
collision_on_batch_sq(polyselect_poly_header_srcptr header,
		      ...
		      unsigned long number_pr,
		      unsigned long k,
		      unsigned long *curr_nq, polyselect_shash_ptr H)
{
  unsigned nr;
  uint64_t pp;
...
static inline void
collision_on_sq(polyselect_poly_header_srcptr header,
		polyselect_proots_srcptr R, unsigned long c,
		polyselect_shash_ptr H, gmp_randstate_ptr rstate)
{
  unsigned long k, lq;
  polyselect_qroots_t SQ_R;
...
static void newAlgo(mpz_ptr N, unsigned long d, unsigned long idx)
{
  mpz_t ad;
  unsigned long c = 0;
...
#pragma omp critical
  ...
  if (verbose >= 0)
      ...
      if (collisions > 0)
	{
	  ...
	  printf
	      ("# Stat: raw lognorm (nr/min/av/max/std): %lu/%1.2f/%1.2f/%1.2f/%1.2f\n",
	       collisions, min_raw_lognorm, rawmean, max_raw_lognorm,
	       sqrt(var_raw_lognorm / collisions - rawmean * rawmean));
	  printf
	      ("# Stat: raw proj. alpha (nr/min/av/max/std): %lu/%1.3f/%1.3f/%1.3f/%1.3f\n",
	       collisions, raw_proj_alpha->min,
	       polyselect_data_mean(raw_proj_alpha), raw_proj_alpha->max,
	       sqrt(polyselect_data_var(raw_proj_alpha)));
	  printf
	  ...
	  if (collisions_good > 0)
	    {
	      ...
	      printf
		  ("# Stat: optimized lognorm (nr/min/av/max/std): %lu/%1.2f/%1.2f/%1.2f/%1.2f\n",
		   collisions_good, polyselect_data_opt_lognorm->min, mean,
		   polyselect_data_opt_lognorm->max,
		   sqrt(polyselect_data_var(polyselect_data_opt_lognorm)));
	      printf
		  ("# Stat: opt proj. alpha (nr/min/av/max/std): %lu/%1.3f/%1.3f/%1.3f/%1.3f\n",
		   collisions_good, opt_proj_alpha->min,
		   polyselect_data_mean(opt_proj_alpha),
		   opt_proj_alpha->max,
		   sqrt(polyselect_data_var(opt_proj_alpha)));
	       ...
	       */
	      printf
		  ("# Stat: discarded %lu polynomials because f[d-1]*f[d-3] > 0\n",
		   discarded2);
	      printf
		  ("# Stat: exp_E (nr/min/av/max/std): %lu/%1.2f/%1.2f/%1.2f/%1.3f\n",
