...
template <int LEVEL, typename HINT> struct bucket_slice_alloc_defaults {
...
template <int LEVEL, typename HINT> class bucket_array_t : private NonCopyable
{
  ...
  private:
    ...
    void log_this_update(update_t const update, uint64_t offset,
  ...
  public:
    ...
    void
    allocate_memory(las_memory_accessor & memory, uint32_t const _n_bucket,
                    double const fill_ratio, int logI,
                    slice_index_t const prealloc_slices = initial_slice_alloc);
  ...
  private:
    ...
    update_t const * begin(size_t const i_bucket,
                           slice_index_t const i_slice) const
    {
        ASSERT_ALWAYS(i_slice < nr_slices);
    ...
    /* Return an end iterator over the update_t entries in i_bucket-th
       bucket, generated by the i_slice-th slice */
    update_t const * end(size_t const i_bucket,
                         slice_index_t const i_slice) const
    {
        ASSERT_ALWAYS(i_slice < nr_slices);
  ...
  public:
    ...
    void add_slice_index(slice_index_t const new_slice_index)
    {
    ...
    inline void push_update(int const i, update_t const & update);

    /* Create an update for a hit at location offset and push it to the
       coresponding bucket */
    inline void push_update(uint64_t const offset, fbprime_t const p,
                            slice_offset_t const slice_offset,
                            slice_index_t const slice_index, where_am_I & w);

    template <typename hh = HINT>
    inline typename std::enable_if<std::is_same<hh, emptyhint_t>::value,
                                   void>::type
    push_update(uint64_t const offset, where_am_I & w MAYBE_UNUSED)
    {
    ...
    inline
        ...
        push_update(uint64_t const offset, logphint_t const & logp,
...
class bucket_array_complete : public bucket_single<1, longhint_t>
{
  ...
  private:
    ...
    void
    purge_1(const bucket_array_t<1, HINT> & BA, const int i,
