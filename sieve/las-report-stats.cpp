...
void las_report::display_survivor_counters() const
{
    ...
    verbose_fmt_print(0, 2, "# survivors before_sieve: {}", S.before_sieve);
    verbose_fmt_print(0, 2, "# survivors after_sieve: {} (ratio {:.2e})\n", S.after_sieve, double_ratio(S.after_sieve, S.before_sieve));
    verbose_fmt_print(0, 2, "# survivors not_both_even: {}", S.not_both_even);
    verbose_fmt_print(0, 2, "# survivors not_both_multiples_of_p: {}", S.not_both_multiples_of_p);
    ...
    for(int pside = 0 ; pside < 2 ; pside++) {
        ...
        if (s && !sx) {
            ...
            verbose_fmt_print(0, 2, "# survivors trial_divided_on_side[{}]: {}", side, sx);
    ...
    verbose_fmt_print(0, 2, "# survivors enter_cofactoring: {}", S.enter_cofactoring);
    verbose_fmt_print(0, 2, "# survivors cofactored: {} ({:.1f}%%)\n", S.cofactored, 100.0 * double_ratio(S.cofactored, S.enter_cofactoring));
    verbose_fmt_print(0, 2, "# survivors smooth: {}", S.smooth);
}
