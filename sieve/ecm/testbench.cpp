...
./testbench -cof 340282366920938463463374607431768211507 -p -ecm 315 5355 11 3
10000000
...
static void print_pointorder(unsigned long const p,
...
static void print_help(char const * programname)
{
    ...
    printf("Run factoring method on numbers in interval [<start>, <stop>], or "
           "from file\n");
    ...
    printf("-ecm <B1> <B2> <s>  Run ECM with B1, B2 and parameter s, using "
           "BRENT12 curve\n");
    printf(
        "-ecmm12 <B1> <B2> <s>  Same, but using Montgomery torsion 12 curve\n");
    printf(
        "-ecmm16 <B1> <B2> <s>  Same, but using Montgomery torsion 16 curve\n");
    printf(
        "-ecmem12 <B1> <B2> <s>  Same, but using Edwards torsion 12 curve\n");
    printf("-strat   Use the facul default strategy. Don't use with -pm1, "
           "-pp1, -ecm\n");
    printf(
        "-fbb <n> Use <n> as factor base bound, e.g. for primality checks\n");
    printf("-lpb <n> Use <n> as large prime bound, e.g. for early abort\n");
    printf(
        "-ep      Add certain extra primes in ECM stage 1 (e.g., 12 or 16)\n");
    printf("         Affects only curves specified after the -ep parameter\n");
    printf("-p       Try only primes in [<start>, <stop>] (default: all odd "
           "numbers)\n");
    printf("-q       Suppress normal output, output from -v, -vf and -vnf "
           "still appears\n");
    printf(
        "-v       More verbose output. Once: print statistics. Twice: print \n"
        "         numbers that are tried. Three: internal info from "
        "strategies\n");
    ...
    printf("-po <s>, -pom12 <s>, -pom16 <s>, -poem12 <s> Compute order of "
           "starting point. Use -vf\n");
}

static unsigned long next_prime(unsigned long start)
{
    mpz_t p;
...
int main(int argc, char const * argv[])
{
    ...
    if (strat) {
        ...
        strategy =
            facul_strategy_oneside(fbb, lpb, 3 * lpb, ncurves, (verbose / 3));
    } else {
        strategy = facul_strategy_oneside(fbb, lpb, 3 * lpb, method_params,
                                          (verbose / 3));
    }
    if (!quiet)
