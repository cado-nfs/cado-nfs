...
enum pp1_parameterization_t {
  PP1_2_7, /* starting point 2/7, leads to group order divisible by 6 */
  PP1_6_5  /* starting point 6/5, leads to group order divisible by 4 */
};

typedef struct {
  char *bc;             /* Bytecode for the Lucas chain for stage 1 */
  enum pp1_parameterization_t parameterization; /* Which starting point to use */
...
void pp1_stage2_mpz (residuempz_t, const residuempz_t, 
                    ...
                    const modulusmpz_t);
void pp1_make_plan (pp1_plan_t *, const unsigned int, const unsigned int,
                    enum pp1_parameterization_t, int);
